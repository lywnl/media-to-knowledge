...
def _limits(
    *,
    ...
    return contracts.EvidencePreparationLimits(
...
def _subtitle(evidence_id: str, start_ms: int, end_ms: int, text: str = "字幕") -> SubtitleCue:
    return SubtitleCue(
...
def _build(
    ...
    module = importlib.import_module("video_demo.application.chapter_planning")
    return module.build_base_segments(
...
def test_sentence_and_silence_candidates_become_safe_boundaries() -> None:
    segments = _build(
        40_000,
        boundaries=(
            contracts.SpeechBoundaryCandidate(10_000, "sentence_end"),
            contracts.SpeechBoundaryCandidate(20_000, "silence"),
        ),
    )
