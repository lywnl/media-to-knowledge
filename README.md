...
视频 ASR 在视频阶段执行器内直接执行，以隔离单个任务的 FFmpeg 和网络调用故障；音频 ASR 在音频专用阶段执行器内运行。两条链路都会复用窗口快照，在同一 Run 重试时只补传未完成窗口；全部成功后再发布整段 ASR 快照。云端最终失败会使对应媒体的 ASR 阶段失败，不会发布空转写或部分成功结果。上传 WAV 是可再生临时产物，窗口完成、失败、超时或取消后都会清理。
