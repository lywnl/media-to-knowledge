...
from video_demo.domain.legacy_result import VideoUnderstandingResult
...
class ResultArtifactPayload(FrozenModel):
    ...
    def validate_transcript_source_evidence(self) -> ResultArtifactPayload:
        subtitle_cues = tuple(
            item for item in self.evidence if isinstance(item, SubtitleCue)
        )
        asr_evidence = tuple(
            item for item in self.evidence if isinstance(item, SpeechSegment)
        )
