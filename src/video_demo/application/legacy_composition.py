...
from video_demo.application.pipeline_contracts import EvidencePreparationLimits, PreparedMedia
...
from video_demo.application.production_visual import ProductionVisualAnalyzer as VisualAnalyzer
...
class ProductionPipeline:
    """仅保留旧诊断组件所有权；2.0 Pipeline 已从生产执行路径切断。"""
    ...
    def run(self, _context: object) -> Never:
        raise VideoDemoError(
            ErrorCode.RESULT_SCHEMA_UNSUPPORTED,
            "旧 2.0 Pipeline 已从生产执行路径切断",
        )
...
def build_production_diagnostic_components(
    ...
    *,
    ...
    _require_legacy_qwen_configuration(settings)
    _require_legacy_baidu_configuration(settings)
...
def _require_legacy_qwen_configuration(settings: Settings) -> None:
    presence = (
        bool(settings.qwen_base_url and settings.qwen_base_url.strip()),
        bool(settings.qwen_model_id and settings.qwen_model_id.strip()),
        bool(settings.qwen_api_key and settings.qwen_api_key.get_secret_value().strip()),
    )
    ...
    endpoint = _normalized_endpoint(settings.qwen_base_url)
...
def _require_legacy_baidu_configuration(settings: Settings) -> None:
    presence = (
        bool(settings.baidu_api_key and settings.baidu_api_key.get_secret_value().strip()),
        bool(settings.baidu_secret_key and settings.baidu_secret_key.get_secret_value().strip()),
    )
    if any(presence) and not all(presence):
        raise VideoDemoError(
            ErrorCode.INVALID_CONFIGURATION,
    ...
    if not settings.baidu_ocr_endpoint.startswith("https://"):
        raise VideoDemoError(
            ErrorCode.INVALID_CONFIGURATION,
            "旧百度 OCR 诊断端点非法",
        )
