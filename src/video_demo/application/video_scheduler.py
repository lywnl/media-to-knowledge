...
class VideoStageExecutor(Protocol):
    """调度器只依赖阶段执行器，不感知 Pipeline 内部细节。"""
...
class VideoTaskScheduler:
    """FastAPI 进程内的视频双阶段调度器。

    队列结构沿用 BiliSum 的 ``deque + Condition + pending_ids/running_ids``，
    但阶段事实由调用方保存到数据库，进程重启时通过 ``recover`` 重建内存队列。
    """

    def __init__(
        self,
        executor: VideoStageExecutor,
        *,
        ...
        if transcription_concurrency < 1 or llm_concurrency < 1:
            raise ValueError("视频阶段并发数必须大于 0")
        ...
        with self._condition:
            ...
            self._dispatch_threads = [
                Thread(
                    ...
                    daemon=True,
                    name="video-transcription-dispatcher",
                ),
                Thread(
                    target=self._dispatch_loop,
                    args=(self._llm, self._run_llm),
                    daemon=True,
                    name="video-llm-dispatcher",
                ),
            ]
            for thread in self._dispatch_threads:
                thread.start()
        self._logger.info(
            "video scheduler started transcription_concurrency=%d llm_concurrency=%d",
    ...
    def submit(
        ...
        self._logger.info("video scheduler enqueue run_id=%s stage=%s", run_id, queue.name)
        return "accepted"

    def _queue_for_stage(self, stage: VideoStageName) -> _StageQueue:
        if stage == VideoStageName.TRANSCRIPTION:
            return self._transcription
        if stage == VideoStageName.LLM:
            return self._llm
        raise ValueError(f"视频阶段不受调度器支持：{stage}")

    def recover(
        self,
        items: tuple[
            tuple[Scope, str] | tuple[Scope, str, VideoStageName],
            ...,
        ] = (),
    ) -> int:
        """把数据库扫描出的未完成 Run 恢复到合适阶段。"""
        ...
        for item in items:
            ...
            checkpoint = None
            if requested_stage != VideoStageName.TRANSCRIPTION:
                try:
                    checkpoint = self._executor.load_checkpoint(scope, run_id)
                    if requested_stage == VideoStageName.LLM and checkpoint is None:
                        error = VideoDemoError(
                    ...
                    self._mark_recovery_failed(scope, run_id, requested_stage, error)
                    self._logger.error(
                        "video scheduler recovery failed run_id=%s stage=%s error_code=%s",
                        run_id,
                        requested_stage or "UNKNOWN",
                        error.code,
                    )
            ...
            with self._condition:
                ...
                self._condition.notify_all()
            self._logger.info(
                "video scheduler recovered run_id=%s stage=%s",
                run_id,
                target.name,
            )
        self._logger.info("video scheduler recovered count=%d", recovered)
        return recovered
        ...
        if callable(handler):
            try:
                ...
                self._logger.exception(
                    "video scheduler recovery failure recording crashed run_id=%s stage=%s",
                    run_id,
                    stage.value,
                )
    ...
    def shutdown(self, *, wait: bool = False, timeout: float | None = None) -> None:
        ...
        if callable(close):
            close()
        pipeline = getattr(self, "_owned_pipeline", None)
        if pipeline is not None and pipeline is not self._executor:
            pipeline_close = getattr(pipeline, "close", None)
            if callable(pipeline_close):
                pipeline_close()
        self._logger.info("video scheduler stopped")
        ...
        while True:
            ...
            if self._is_cancelled(scope, run_id):
                with self._condition:
                    ...
                    self._condition.notify_all()
                self._logger.info(
                    "video scheduler skip cancelled run_id=%s stage=%s",
                    run_id,
                    queue.name,
                )
                continue
            ...
            thread = Thread(
                ...
                daemon=True,
                name=f"video-{queue.name.lower()}-{run_id}",
            )
            with self._condition:
                self._job_threads.add(thread)
                slot = len(queue.running_ids)
            self._logger.info(
                "video scheduler start run_id=%s stage=%s slot=%d/%d",
                run_id,
                queue.name,
                slot,
                queue.concurrency,
        ...
        with self._condition:
            while True:
                ...
                if queue.pending and len(queue.running_ids) < queue.concurrency:
                    ...
                    self._logger.info(
                        "video scheduler dequeue run_id=%s stage=%s pending=%d running=%d/%d",
                        run_id,
                        queue.name,
                        len(queue.pending),
    ...
    def _execute(
        ...
        try:
            ...
            if queue is self._transcription and result is not None:
                with self._condition:
                    if not self._shutdown_requested:
                        ...
                        self._condition.notify_all()
                self._logger.info("video scheduler handoff run_id=%s stage=LLM", run_id)
            else:
                with self._condition:
            ...
            self._logger.warning(
                "video scheduler stage failed run_id=%s stage=%s error_code=%s retry=%s",
                run_id,
                queue.name,
                error.code,
                retry,
            ...
            with self._condition:
                ...
                if retry and not self._shutdown_requested:
                    Thread(
                        ...
                        daemon=True,
                        name=f"video-{queue.name.lower()}-retry-{run_id}",
                    ).start()
                self._condition.notify_all()
            self._logger.info(
                "video scheduler finish run_id=%s stage=%s duration_ms=%d retry=%s running=%d/%d",
                run_id,
                queue.name,
    ...
    def _enqueue_locked(self, queue: _StageQueue, scope: Scope, run_id: str) -> None:
        ...
        queue.pending.append((scope, run_id))
        queue.pending_ids.add(key)
        self._logger.info("video scheduler retry enqueue run_id=%s stage=%s", run_id, queue.name)
    ...
    def _run_llm(self, scope: Scope, run_id: str) -> object:
        ...
        if checkpoint is None:
            raise VideoDemoError(ErrorCode.VIDEO_RESULT_NOT_READY, "转写阶段快照不存在")
    ...
    def _record_failure(
        ...
        try:
            ...
            self._logger.exception(
                "video scheduler failure recording crashed run_id=%s stage=%s",
                run_id,
                stage,
            )
            return False
