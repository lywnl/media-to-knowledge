...
class AudioJobHandler:
    ...
    def __call__(self, job: ClaimedJob) -> None:
        ...
        try:
            ...
            if isinstance(artifact, NoAudioArtifact):
            ...
            outcome = self._pipeline_factory(probe.duration_ms).run(
                ...
                source=self._runtime_root / artifact.relative_path,
                duration_ms=probe.duration_ms,
    ...
    def _load_input(self, job: ClaimedJob) -> tuple[Path, AudioRunConfig, str, str]:
        with self._database.session() as session:
            ...
            source = self._object_store.materialize(job.scope, record, job.resource_id, obj.sha256)
            try:
                config = AudioRunConfig.model_validate(run.config_snapshot)
    ...
    def _is_cancel_requested(self, job: ClaimedJob) -> bool:
        with self._database.session() as session:
            return JobRepository(session).is_cancel_requested(
                job.id, job.worker_id, attempt_count=job.attempt_count
            )
